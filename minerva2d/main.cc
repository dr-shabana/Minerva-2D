...
void resetRotation()
{
    ...
    if (!pSetDisplayAutoRotationPreferences) {
        dbgMinerva << "Failed to load function SetDisplayAutoRotationPreferences";
        return;
    }
    bool result = pSetDisplayAutoRotationPreferences(ORIENTATION_PREFERENCE_NONE);
    dbgMinerva << "SetDisplayAutoRotationPreferences(ORIENTATION_PREFERENCE_NONE) returned" << result;
}
} // namespace
#endif
...
Java_org_minerva2d_android_JNIWrappers_saveState(JNIEnv* /*env*/,
                                             jobject /*obj*/,
                                             jint    /*n*/)
{
    ...
    minerva2drc.setValue("canvasState", "OPENGL_SUCCESS");
}

extern "C" JNIEXPORT jboolean JNICALL
Java_org_minerva2d_android_JNIWrappers_hasMainWindowLoaded(JNIEnv * /*env*/,
                                                       jobject /*obj*/,
                                                       jint /*n*/)
{
    if (!KisPart::exists()) {
...
extern "C" JNIEXPORT void JNICALL
Java_org_minerva2d_android_JNIWrappers_openFileFromIntent(JNIEnv* /*env*/,
                                                      jobject /*obj*/,
                                                      jstring str)
{
...
extern "C" MAIN_EXPORT int MAIN_FN(int argc, char **argv)
{
    ...
    QString key = "Minerva5" + QStandardPaths::writableLocation(QStandardPaths::HomeLocation).replace("/", "_");
    ...
    if (resultHash[write_permission] == QtAndroid::PermissionResult::Denied) {
        ...
        dbgMinerva << "Permission denied by the user";
    }
    else {
        dbgMinerva << "Permission granted";
    }
 ...
 */
    ...
    // The following code should be configured to **only** run when we detect that Minerva is being
    ...
    QSettings minerva2drc(configPath.absoluteFilePath("kritadisplayrc"), QSettings::IniFormat);
    ...
    bool logUsage = true;
    {
        bool enableHighDpiScaling = minerva2drc.value("EnableHiDPI", true).toBool() || !qgetenv("MINERVA2D_HIDPI").isEmpty();
...
#endif
#if defined(Q_OS_WIN) && defined(HAVE_HIGH_DPI_SCALE_FACTOR_ROUNDING_POLICY)
        if (minerva2drc.value("EnableHiDPIFractionalScaling", false).toBool()) {
    ...
    if (!qEnvironmentVariableIsSet("QT_XCB_GL_INTEGRATION")) {
        if (KisConfig::preferXcbEglProvider(&minerva2drc)) {
            qputenv("QT_XCB_GL_INTEGRATION", "xcb_egl");
        }
    }
#endif

if (!qEnvironmentVariableIsEmpty("MINERVA2D_OPENGL_DEBUG")) {
            enableOpenGLDebug = true;
        } else {
            enableOpenGLDebug = minerva2drc.value("EnableOpenGLDebug", false).toBool();
        }
        if (enableOpenGLDebug && (qgetenv("MINERVA2D_OPENGL_DEBUG") == "sync" || minerva2drc.value("OpenGLDebugSynchronous", false).toBool())) {
            openGLDebugSynchronous = true;
        }

        KisConfig::RootSurfaceFormat rootSurfaceFormat = KisConfig::rootSurfaceFormat(&minerva2drc);
        KisOpenGL::OpenGLRenderer preferredRenderer = KisOpenGL::RendererAuto;

        logUsage = minerva2drc.value("LogUsage", true).toBool();

#ifdef Q_OS_WIN
        const QString preferredRendererString = minerva2drc.value("OpenGLRenderer", "angle").toString();
#else
        const QString preferredRendererString = minerva2drc.value("OpenGLRenderer", "auto").toString();
#endif
        ...
        const KisConfig::CanvasSurfaceBitDepthMode bitDepthMode =
            KisConfig::canvasSurfaceBitDepthMode(&minerva2drc);
    ...
    if (!qEnvironmentVariableIsSet("QT_DISABLE_ACCESSIBILITY")) {
        if (minerva2drc.value("DisableAccessibilityInQt", false).toBool()) {
...
#ifdef Q_OS_WIN
        // HACK: https://github.com/dr-shabana/Minerva-2D/issues/show_bug.cgi?id=390651
        resetRotation();
#endif
    }
	...
	qputenv("MINERVA2D_PLUGIN_PATH", QString(QFile::encodeName(root + "lib")).toUtf8());
    ...
    dbgMinerva << "Setting XDG_DATA_DIRS" << qgetenv("XDG_DATA_DIRS");
...
#ifdef Q_OS_MAC
                // See https://github.com/dr-shabana/Minerva-2D/issues/show_bug.cgi?id=396370
    ...
    const bool forceWinTab = !KisConfig::useWin8PointerInputNoApp(&minerva2drc);
    ...
    /**
     * In Qt5 version of Minerva we used to patch ki18n to look for translations
     ...
     * into the default location, i.e. minerva2d.appdir/usr/share/locale.
     *
    ...
    KAboutData aboutData("krita",
                            i18n("Minerva"),
                            MinervaVersionWrapper::versionString(true),
                            i18n("Minerva is the full-featured digital art studio"),
                            KAboutLicense::GPL,
                            i18nc("@info:credit", "© 1999–2026 The Minerva Developers"));
    aboutData.setHomepage(QStringLiteral("https://minerva2d.org"));
    aboutData.setOrganizationDomain("minerva2d.org");
    ...
    // paths of the form of minerva2d/minerva2d, which is weird.
    ...
    dbgMinerva << "PATH" << qgetenv("PATH");
#endif

    if (KisApplication::applicationDirPath().contains(MINERVA2D_BUILD_DIR)) {
        qFatal("FATAL: You're trying to run krita from the build location. You can only run Minerva from the installation location.");
    }
    ...
    KisUsageLogger::writeSysInfo(QString("  GPU Acceleration: %1").arg(minerva2drc.value("OpenGLRenderer", "auto").toString()));
    ...
    if (!app.start(args)) {
        KisUsageLogger::log("Could not start Minerva Application");
        return 1;
    }
...
void installPythonPluginUITranslator(KisApplication &app)
{
    ...
    // These UI files must specify "pyminerva2d_plugin_ui" as their class names.
    KLocalizedTranslator *translator = new KLocalizedTranslator(&app);
    translator->setObjectName(QStringLiteral("KLocalizedTranslator.pyminerva2d_plugin_ui"));
    translator->setTranslationDomain(QStringLiteral("krita"));
    translator->addContextToMonitor(QStringLiteral("pyminerva2d_plugin_ui"));
