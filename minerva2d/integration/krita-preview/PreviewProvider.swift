...
//  Copyright © 2025 Stichting Minerva Foundation. All rights reserved.
//
...
#if DEBUG
        NSLog("minerva2d-preview loaded file: \(request.fileURL.path)")
        for representation in appIcon!.representations {
            if let bitmapRep = representation as? NSBitmapImageRep {
                NSLog("minerva2d-preview bmp: \(bitmapRep.pixelsWide)x\(bitmapRep.pixelsHigh)")
                // Access bitmap data, properties, etc.
            } else {
                NSLog("minerva2d-preview: no representations")
            }
        }
#endif
