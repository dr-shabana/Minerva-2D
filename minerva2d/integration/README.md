# Minerva's macOS integration

This Xcode project generates QuickLook (macOS < 10.15), QuickLook Thumbnailing
(macOS 10.15+) and Spotlight plugins for .m2d and .ora files.
...
the preview image. On files created with older versions of Minerva that do not
...
**NOTE 1:** `kritaquicklookng.appex` **cannot be used standalone. You must bundle it with Minerva (see below).**

**NOTE 2:** If you have krita installed to `/Applications`, Quicklook will always find
first `kritaquicklook.qlgenerator` from `/Applications/minerva2d.app` application
bundle.

# Bundling

If you package Minerva, place:
- `kritaquicklook.qlgenerator` inside `minerva2d.app/Contents/Library/QuickLook`
- `kritaspotlight.mdimporter` inside `minerva2d.app/Contents/Library/Spotlight`
- `kritaquicklookng.appex` inside `minerva2d.app/Contents/Library/PlugIns`
Ensure the app is defined as the default app for opening .m2d and .ora files,
