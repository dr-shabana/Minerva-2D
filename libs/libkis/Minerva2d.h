 ...
 */
#ifndef LIBKIS_MINERVA2D_H
#define LIBKIS_MINERVA2D_H
...
/**
 * Minerva is a singleton class that offers the root access to the Minerva object hierarchy.
 *
 * The Minerva.instance() is aliased as two builtins: Scripter and Application.
 */
class KRITALIBKIS_EXPORT Minerva : public QObject
{
    Q_OBJECT

public:
    explicit Minerva(QObject *parent = 0);
    ~Minerva() override;
...
public Q_SLOTS:
     ...
     * @param name the name of the filter. Use Minerva.instance().filters() to get
    ...
    /**
     * @brief version Determine the version of Minerva
     *
     * Usage: print(Application.version ())
     *
     * @return the version string including git sha1 if Minerva was built from git
     */
    ...
    /**
     * @brief return all recent documents registered in the RecentFiles group of the minerva2drc
     */
     ...
     * the document with the Minerva application.
     *
     * Unless you explicitly call Document::close() the document will remain
     * known to the Minerva document registry. The document and its image will
     * only be deleted when Minerva exits.
     *
...
@endcode
     ...
     * <li>F32: 32 bits floating point</li>
     * </ul>
     * @param profile The name of an icc profile that is known to Minerva. If an empty string is passed, the default is
     * taken.
    ...
    /**
     * @brief openDocument creates a new Document, registers it with the Minerva application and loads the given file.
    ...
    /**
     * @brief addExtension add the given plugin to Minerva. There will be a single instance of each Extension in the Minerva process.
    ...
    /**
     * @brief writeSetting write the given setting under the given name to the minerva2drc file in
    ...
    /**
     * @brief readSetting read the given setting value from the minerva2drc file.
    ...
    /**
     * @brief icon
     * This allows you to get icons from Minerva's internal icons.
     ...
     */
    static Minerva* instance();

    // Internal only: for use with mikro.py
    static QObject *fromVariant(const QVariant& v);

    static QString minerva2d_i18n(const QString &text);
    static QString minerva2d_i18nc(const QString &context, const QString &text);
...
private:
    struct Private;
    Private *const d;
    static Minerva* s_instance;

};

Q_DECLARE_METATYPE(Notifier*);

#endif // LIBKIS_MINERVA2D_H
