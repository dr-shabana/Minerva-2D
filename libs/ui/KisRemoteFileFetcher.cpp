...
bool KisRemoteFileFetcher::fetchFile(const QUrl &remote, QIODevice *io)
{
    ...
    m_request->setRawHeader("User-Agent", QString("Minerva-%1").arg(qApp->applicationVersion()).toUtf8());
    m_reply = manager.get(*m_request);

    QLocale loc;

    QProgressDialog progress;
    progress.setWindowTitle(i18nc("@title:window", "Minerva"));
...
QByteArray KisRemoteFileFetcher::fetchFile(const QUrl &remote)
{
    ...
    request->setRawHeader("User-Agent", QString("Minerva-%1").arg(qApp->applicationVersion()).toUtf8());
