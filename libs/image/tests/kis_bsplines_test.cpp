...
void KisBSplinesTest::test1D()
{
    ...
    for (qreal x = start; x < end; x += 0.01) {
        ...
        if (relError > 0.10) {
            dbgMinerva << ppVar(x) << ppVar(op(x)) << ppVar(spline.value(x)) << ppVar(relError);
        }
    }
...
bool test2DSpline(const Spline &spline, const Op &op, qreal start, qreal end)
{
    ...
    for (qreal y = start; y < end; y += 0.01) {
        for (qreal x = start; x < end; x += 0.01) {
            ...
            if (relError > 0.10) {
                dbgMinerva << ppVar(x) << ppVar(y) << ppVar(op(x, y)) << ppVar(spline.value(x, y)) << ppVar(relError);
            }
        }
    }
