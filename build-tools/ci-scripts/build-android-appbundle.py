...
parser = argparse.ArgumentParser(description='A script for building Minerva Android AppBundle package on CI')
...
buildEnvironment['MINERVA2D_BUILD_APPBUNDLE'] = '1'
buildEnvironment['APK_PATH'] = artifactsFolder
buildEnvironment['MINERVA2D_INSTALL_PREFIX'] = '.xxx' # just a fake path to trigger an error if used (if shouldn't be used for aab)
...
try:
    ...
    # TODO: if we need x86
    xml_paths = [f'{packagingFolder}/minerva2d_build_apk/res/values/libs-{arch}.xml'
                 for arch in ['arm64-v8a', 'x86_64', 'armeabi-v7a']]
    xml_paths_str = ",".join(xml_paths)
    subprocess.check_call(f'{sys.executable} merge-libs-xml.py -p {xml_paths_str} --output {packagingFolder}/minerva2d_build_apk/res/values/libs.xml',
