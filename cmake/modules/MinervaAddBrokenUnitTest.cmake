...
function(_minerva2d_test_name_prefix_from_project_path _name_prefix_varname)
...
function(MINERVA2D_ADD_UNIT_TEST)
  ...
  if (NOT ARG_NAME_PREFIX)
      _minerva2d_test_name_prefix_from_project_path(ARG_NAME_PREFIX)
  endif()
  ...
  if(ARG_BROKEN)
      ...
      set(MINERVA2D_BROKEN_TESTS ${MINERVA2D_BROKEN_TESTS} ${_testname} CACHE INTERNAL "MINERVA2D_BROKEN_TESTS")
  endif()
  # add test to global test list so we can operate on all targets at once
  set(MINERVA2D_TESTS_TARGET ${MINERVA2D_TESTS_TARGET} ${_targetname} CACHE INTERNAL "MINERVA2D_TESTS_TARGET")
  ...
  if (MINERVA2D_ENABLE_PCH AND ARG_PCH_FILE)
  ...
  if(NOT ARG_BROKEN OR MINERVA2D_ENABLE_BROKEN_TESTS)
...
function(MINERVA2D_ADD_UNIT_TESTS)
  set(options GUI BROKEN)
  ...
  foreach(_test_source ${ARG_UNPARSED_ARGUMENTS})
    MINERVA2D_ADD_UNIT_TEST(${_test_source}
    ...
    if (MINERVA2D_ENABLE_PCH AND ${_testsCount} GREATER "2")
        set_property(TARGET ${_targetname} PROPERTY PCH_WARN_INVALID TRUE )
        ...
        if (NOT pch_source_target)
            ...
            if (NOT ARG_PCH_FILE)
                minerva2d_select_pch_file(${target_name} ARG_PCH_FILE)
...
macro(MINERVA2D_ADD_BROKEN_UNIT_TEST)
    MINERVA2D_ADD_UNIT_TEST(${ARGV} BROKEN)
endmacro()

macro(MINERVA2D_ADD_BROKEN_UNIT_TESTS)
    MINERVA2D_ADD_UNIT_TESTS(${ARGV} BROKEN)
endmacro()

macro(kis_add_tests)
    MINERVA2D_ADD_UNIT_TESTS(${ARGV})
endmacro()

macro(kis_add_test)
    MINERVA2D_ADD_UNIT_TEST(${ARGV})
endmacro()
...
function(kis_add_tests_maybe_broken)
    ...
    MINERVA2D_ADD_UNIT_TESTS(${ARG_UNPARSED_ARGUMENTS} ${_extra_args})
endfunction()
...
function(kis_add_test_maybe_broken)
    ...
    MINERVA2D_ADD_UNIT_TEST(${ARG_UNPARSED_ARGUMENTS} ${_extra_args})
endfunction()
