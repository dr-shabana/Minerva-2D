...
if [ -z "${MINERVA2D_BRANDING}" ]; then
    # determine the channel for branding
    if [ "${JOB_NAME}" == "Minerva_Nightly_Appimage_Build" ]; then
        MINERVA2D_BRANDING="Next"
    elif [ "${JOB_NAME}" == "Minerva_Stable_Appimage_Build" ]; then
        MINERVA2D_BRANDING="Plus"
    else
        MINERVA2D_BRANDING=""
    fi
fi
...
if [ ! -d $BUILD_PREFIX/minerva2d-build/ ] ; then
    mkdir -p $BUILD_PREFIX/minerva2d-build/
fi

# Now switch to it
cd $BUILD_PREFIX/minerva2d-build/
...
# Configure Minerva
cmake $MINERVA2D_SOURCES \
    -DCMAKE_INSTALL_PREFIX:PATH=$BUILD_PREFIX/minerva2d.appdir/usr \
    -DDEFINE_NO_DEPRECATED=1 \
    -DCMAKE_BUILD_TYPE=${BUILD_TYPE} \
    -DFOUNDATION_BUILD=1 \
    -DHIDE_SAFE_ASSERTS=ON \
    -DBUILD_TESTING=FALSE \
    -DMINERVA2D_ENABLE_PCH=off \
    -DPYQT_SIP_DIR_OVERRIDE=$DEPS_INSTALL_PREFIX/share/sip/ \
    -DHAVE_MEMORY_LEAK_TRACKER=FALSE \
    -DBRANDING="${MINERVA2D_BRANDING}"

# Build and Install Minerva (ready for the next phase)
cmake --build . --target install --parallel $CPU_COUNT

# We add Minerva's AppImage location for plugins (GMic)
export PLUGINS_INSTALL_PREFIX=$BUILD_PREFIX/minerva2d.appdir/usr
...
cmake $MINERVA2D_SOURCES/3rdparty_plugins \
