...
bool DlgCreateBundle::putResourcesInTheBundle(KoResourceBundleSP bundle)
{
    ...
    while(!allResourcesIds.isEmpty()) {
        ...
        if (!res) {
            warnMinerva << "No resource for id " << id;
            continue;
        }
...
void DlgCreateBundle::accept()
{
    ...
    if (name.isEmpty()) {
        ...
        QMessageBox::warning(this, i18nc("@title:window", "Minerva"), i18n("The resource bundle name cannot be empty."));
        return;
    }
    ...
    if (m_pageBundleSaver->saveLocation().isEmpty() ){
        ...
        QMessageBox::warning(this, i18nc("@title:window", "Minerva"), i18n("The save location cannot be empty."));
        return;
    }
    else {
