...
// config.h defines PYMINERVA2D_PYTHON_LIBRARY, the path to libpython.so
// on the build system
...
namespace PyMinerva
{
    ...
    InitResult initialize()
    {
        ...
        if (0 != PyImport_AppendInittab(Python::PYMINERVA2D_ENGINE, PYMINERVA2D_INIT)) {
            initStatus = INIT_CANNOT_LOAD_PYMINERVA2D_MODULE;
            return initStatus;
        }
        ...
        // Initialize 'plugins' dict of module 'pyminerva2d'
        ...
        auto pyminerva2dModule = PYMINERVA2D_INIT();

        if (!pyminerva2dModule) {
            initStatus = INIT_CANNOT_LOAD_PYMINERVA2D_MODULE;
            return initStatus;
            //return i18nc("@info:tooltip ", "No <icode>pyminerva2d</icode> built-in module");
        }
    ...
    void finalize() {
        ...
        if (pluginManagerInstance) {
            ...
            PyMinerva::Python::maybeFinalize();
            PyMinerva::Python::libraryUnload();
...
const char* Python::PYMINERVA2D_ENGINE = "pyminerva2d";
...
bool Python::libraryLoad()
{
    ...
    if (!s_pythonLibrary) {
        ...
        QFileInfo fi(PYMINERVA2D_PYTHON_LIBRARY);
...
QString findMinervaPythonLibsPath(const QString &libdir)
{
...
//             << (!qgetenv("APPDIR").isNull() && KoResourcePaths::getApplicationRoot().contains(qgetenv("APPDIR")));
    ...
    bool runningInBundle = KoResourcePaths::getApplicationRoot().toLower().contains("minerva2d.app");
#else
    ...
    // Append the Minerva libraries path
    QString pythonLibsPath = findMinervaPythonLibsPath("minerva2d-python-libs");
    dbgScript << "pythonLibsPath (minerva2d-python-libs)" << pythonLibsPath;
    if (pythonLibsPath.isEmpty()) {
        dbgScript << "Cannot find minerva2d-python-libs";
        return false;
    }
    dbgScript << "Found minerva2d-python-libs at" << pythonLibsPath;
    paths.append(pythonLibsPath);

#ifndef Q_OS_WIN
    // Append the sip libraries path
    pythonLibsPath = findMinervaPythonLibsPath("sip");
    ...
    QString distToolsPath = findMinervaPythonLibsPath("site-packages");
    ...
    if (runningInBundle) {
        ...
        QString p = QFileInfo(PYMINERVA2D_PYTHON_LIBRARY).fileName();
#ifdef Q_OS_MAC
...
#endif
        dbgScript << "\t" << p << p2;
        originalPath = findMinervaPythonLibsPath(p);
#ifdef Q_OS_MAC
        ...
        if (originalPath.isEmpty()) {
            ...
            QString d = QFileInfo(PYMINERVA2D_PYTHON_LIBRARY).absolutePath();
