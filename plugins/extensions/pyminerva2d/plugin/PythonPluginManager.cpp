...
{
    dbgScript << "Got Minerva/PythonPlugin: " << name()
...
QPair<QString, PyMinerva::version_checker> PythonPluginManager::parseDependency(const QString& d)
{
    ...
    if (pnfo != -1) {
        ...
        PyMinerva::version_checker checker = PyMinerva::version_checker::fromString(version_str);
        if (!(checker.isValid() && d.endsWith(')'))) {
            ...
            return qMakePair(reason, PyMinerva::version_checker());
        }
        return qMakePair(dependency, checker);
    }
    return qMakePair(d, PyMinerva::version_checker(PyMinerva::version_checker::undefined));
}
...
void PythonPluginManager::verifyDependenciesSetStatus(PythonPlugin& plugin)
{
    ...
    Q_FOREACH(const QString & d, dependencies) {
        QPair<QString, PyMinerva::version_checker> info_pair = parseDependency(d);
        PyMinerva::version_checker& checker = info_pair.second;
        if (!checker.isValid()) {
            plugin.m_broken = true;
        ...
        if (module) {
            ...
            PyMinerva::version dep_version = PyMinerva::version::fromPythonObject(version_obj);
...
void PythonPluginManager::scanPlugins()
{
    ...
    QStringList desktopFiles = KoResourcePaths::findAllAssets("data", "pyminerva2d/*desktop");
...
void PythonPluginManager::loadModule(PythonPlugin &plugin)
{
    ...
    // Get 'plugins' key from 'pyminerva2d' module dictionary.
    ...
    if (module) {
        ...
        if (ins_result == 0) {
            ...
            PyObject* result = py.functionCall("_pluginLoaded", PyMinerva::Python::PYMINERVA2D_ENGINE, args);
            Py_DECREF(args);
            if (result) {
                dbgScript << "\t" << "success!";
...
void PythonPluginManager::unloadModule(PythonPlugin &plugin)
{
    ...
    py.functionCall("_pluginUnloading", PyMinerva::Python::PYMINERVA2D_ENGINE, args);
    Py_DECREF(args);
