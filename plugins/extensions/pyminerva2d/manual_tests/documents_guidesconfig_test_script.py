...
try:
    from PyQt6.QtCore import qFuzzyCompare, qDebug, qWarning
    from PyQt6.QtGui import QColor
except:
    from PyQt5.QtCore import qFuzzyCompare, qDebug, qWarning
...
class TestDocument():
    def __init__(self):
        self.newDoc = Minerva.instance().createDocument(500, 500, "Test autosave", "RGBA", "U8", "", 300)

        with tempfile.TemporaryDirectory() as tmpDirname:
            tmpFileName = os.path.join(tmpDirname, 'testing_document.m2d')
            ...
            self.newDoc = Minerva.instance().openDocument(tmpFileName)
            self.checkTestValues01(self.newDoc)
