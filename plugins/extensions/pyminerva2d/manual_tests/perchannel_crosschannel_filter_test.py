...
def createDocument(fileName, filterName, curves):
    doc = Minerva.instance().createDocument(500, 500, "TestFilter", "RGBA", "U8", "", 72.0)
    #Minerva.instance().activeWindow().addView(doc)

    filter = Minerva.instance().filter(filterName)
    selection = Selection()
...
def modifyDocument(fileName, curves):
    doc = Minerva.instance().openDocument(fileName)
    #Minerva.instance().activeWindow().addView(doc)
...
if Minerva.instance():
    ...
    with tempfile.TemporaryDirectory() as tmpDirname: 
        ...
        tmpFileNamePC = os.path.join(tmpDirname, 'testing_filter_pc.m2d')
        ...
        tmpFileNameCC = os.path.join(tmpDirname, 'testing_filter_cc.m2d')
