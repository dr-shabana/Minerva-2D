...
"""
BBD's Minerva script starter

This script does the boring stuff involved in creating a script for Minerva.
it creates
* a directory for the script in the correct Minerva resources subdirectory,
...
* (optionally) automatically enables the script in the Minerva menu

Script can be run from the command line. This can be used to
bootstrap the script into a Minerva menu entry - create a new script
called Minerva Script Starter, then copy the script (and the .ui file)
...
try:
    import krita
    CONTEXT_KRITA = True
    EXTENSION = minerva2d.Extension

except ImportError:
    # script being run in testing environment without Minerva
    CONTEXT_KRITA = False
    EXTENSION = QWidget
...
MAIN_MINERVA2D_ID = "Minerva Script Starter"
MAIN_MINERVA2D_MENU_ENTRY = "Minerva Script Starter"

SCRIPT_NAME = "script_name"
SCRIPT_COMMENT = "script_comment"
MINERVA2D_ID = "minerva2d_id"
...
Type=Service
ServiceTypes=Minerva/PythonPlugin
X-KDE-Library={library_name}
X-Minerva-Manual=Manual.html
Name={script_name}
...
EXTENSION_TEMPLATE = """# BBD's Minerva Script Starter Feb 2018

from krita import Extension

EXTENSION_ID = '{minerva2d_id}'
MENU_ENTRY = '{menu_entry}'
...
class {class_name}(Extension):
    ...
    def createActions(self, window):
        ...
        # parameter 1 = the name that Minerva uses to identify the action
...
<!--BBD's Minerva Script Starter, Feb 2018 -->
...
class MinervaScriptStarter(EXTENSION):
    ...
    def setup(self):
        ...
        target_directory = Minerva.instance().getAppDataLocation()
        if not CONTEXT_KRITA:
            target_directory = os.path.join(target_directory, "krita")
        target_directory = os.path.join(target_directory, "pyminerva2d")
        self.target_directory = target_directory

    def createActions(self, window):
        """ Called by Minerva to create actions."""
        action = window.createAction(
            MAIN_MINERVA2D_ID, MAIN_MINERVA2D_MENU_ENTRY, "tools/scripts")
        # parameter 1 = the name that Minerva uses to identify the action
        # parameter 2 = the text to be added to the menu entry for this script
        # parameter 3 = location of menu entry
        action.triggered.connect(self.action_triggered)
    ...
    def create(self):
        ...
        values = {
            ...
            MINERVA2D_ID: "pyminerva2d_" + self.package_name,
        ...
        # notify success
        # Assemble message
        title = "Minerva Script files created"
        message = []
...
if __name__ == "__main__":
    ...
    if CONTEXT_KRITA:
        ...
        extension = MinervaScriptStarter(None)
        extension.setup()
