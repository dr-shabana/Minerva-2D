# Batch Exporter: Minerva Plugin for Game Developers and Graphic Designers

Free Minerva plugin for designers, game artists and digital artists to work more
productively:
...
- `[p=path/to/custom/export/directory]` or `[p="path with spaces"]` - custom output path.
  Paths can be absolute or relative to the Minerva document.
...
Minerva document. The export follows the structure of your layer stack. The group
...
Say we have this Minerva document structure:
...
If you want to export multiple or specific COA Tool documents from one Minerva document
(if you have e.g. multiple characters in one Minerva document),
you can do so by selecting a Group Layer to serve as root for each COA Tool export
you want done.

### Example

You want to export two characters from the same Minerva document in one go
