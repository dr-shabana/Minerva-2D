...
void DlgExportStoryboard::slotExportClicked()
{
    if (m_page->exportPathFileRequester->fileName().isEmpty()) {
        if (m_format == ExportFormat::PDF) {
            QMessageBox::warning(this, i18nc("@title:window", "Minerva"), i18n("Please enter a file name to export to."));
        }
        else {
            QMessageBox::warning(this, i18nc("@title:window", "Minerva"), i18n("Please enter a directory to export to."));
        }
        return;
    }
    ...
    if (m_format == ExportFormat::SVG) {
        ...
        if (!dir.exists()) {
            QMessageBox::warning(this, i18nc("@title:window", "Minerva"), i18n("Please enter an existing directory."));
            return;
        }
    ...
    if (layoutSpecifiedBySvgFile() && m_page->svgTemplatePathFileRequester->fileName().isEmpty()) {
        QMessageBox::warning(this, i18nc("@title:window", "Minerva"), i18n("Please choose svg file to specify the layout for exporting."));
        return;
    }
    QFileInfo fi(m_page->svgTemplatePathFileRequester->fileName());
    if (layoutSpecifiedBySvgFile() && !fi.exists()) {
        QMessageBox::warning(this, i18nc("@title:window", "Minerva"), i18n("The SVG file to specify layout doesn't exist. Please choose an existing SVG file."));
        return;
    }
